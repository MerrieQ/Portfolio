...
def extract_score_from_feedback(feedback):
    for word in feedback.split():
        if word.isdigit() and 1 <= int(word) <= 5:
            return int(word)
    return 1  # Fallback score als geen geldige score wordt gevonden
...
def analyze_document_with_llm(document_text):
    ...
    for chapter, item_code, section, item, description in TRIPOD_CHECKLIST:
        ...
        Geef een beoordeling van 1 tot 5 (waarbij 1 = voldoet niet en 5 = voldoet volledig) en leg kort uit waarom je dit cijfer geeft:
        """
        ...
        summary_table.append([item_code, item, score])
...
def generate_pdf_report(results, summary_table):
    ...
    table_data = [["Item Code", "Item", "Score"]]
    for row in summary_table:
        item_code, item, score = row
        table_data.append([item_code, item, str(score)])
